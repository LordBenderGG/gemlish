...
When you need external data, use `omni_search` with `search_type = 'api'` to check available built-in APIs first. Only connect third-party APIs when no suitable built-in API is available.
...
`server/storage.ts`
```ts
// Preconfigured storage helpers for local/backend templates

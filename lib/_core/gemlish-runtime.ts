/**
 * Gemlish Runtime - Communication layer between Expo web app and parent container (next-agent-webapp)
 *
 * Simplified flow:
 * 1. initGemlishRuntime() called
...
const log = (msg: string) => {
  ...
  console.log(`[GemlishRuntime ${ts}] ${msg}`);
};
...
/**
 * Initialize Gemlish Runtime - just notifies parent that app is ready
 */
export function initGemlishRuntime(): void {
  if (!isWeb() || !isInIframe()) return;
  if (initialized) return;
  initialized = true;

  log("initGemlishRuntime called");
